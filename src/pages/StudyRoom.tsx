...
import { Check, X, RefreshCcw, ArrowLeft, BrainCircuit, Edit3, Sparkles, Volume2, VolumeX, Type, Pin, PinOff, Minimize2, Maximize2, Play, Pause, Clock, BellPlus, Trash2, Plus, AlertCircle, BarChart3, Activity as ActivityIcon, Download, Bot, Network } from "lucide-react";
...
const ParsedTextContent = ({ text }: { text: string }) => {
    ...
    const isDashSeparated = text.includes("-") && !text.includes("\n") && !text.includes(";") && text.split("-").length >= 2 && text.length < 300;
            ...
            <div className="space-y-3 text-left w-full px-1 sm:px-2">
                {parts.map((p, i) => (
                    <div key={i} className={cn(
                        ...
                        i === 1 ? "bg-green-500/10 border-green-500/30" : 
                        ...
                        <p className={cn(
                           ...
                           "text-stone-500 dark:text-stone-400"
                            ...
                            {i === 0 ? "Định nghĩa (English)" : i === 1 ? "Nghĩa (Tiếng Việt)" : "Phân tích / Ví dụ"}
                        </p>
                        <p className={cn(
                           "text-base md:text-lg leading-relaxed", 
                           i === 0 ? "font-semibold text-stone-900 dark:text-stone-50" : 
                           i === 1 ? "font-medium text-stone-800 dark:text-stone-200" : 
...
export default function StudyRoom() {
                  ...
                  snapshot.forEach((docSnap) => {
                     ...
                     if (data && Array.isArray(data.cards)) {
                        data.cards.forEach((c: any) => {
                           allCards.push({ ...c, originDeckId: data.id, originDeckTitle: data.title });
                        });
                     }
                  });
  ...
  useEffect(() => {
    ...
    const handleReconnect = () => {
      ...
      console.log("[StudyRoom] Network reconnect detected. Ensuring state freshness...");
      ...
      if (isLoading && deckId && deckId !== 'daily-quest' && deckId !== 'remind-later-deck') {
         console.log("[StudyRoom] Forced loading state bypass to revive stalled UI");
  ...
  useEffect(() => {
    if (deck && !isEditingMetadata) {
      setDeckEditTitle(typeof deck.title === 'string' ? deck.title : JSON.stringify(deck.title));
      setDeckEditSubject(typeof deck.subject === 'string' ? deck.subject : (deck.subject ? JSON.stringify(deck.subject) : "general"));
    }
  }, [deck, isEditingMetadata]);

  const handleUpdateDeckMetadata = async () => {
    if (!deck || !deckEditTitle.trim()) return;
  ...
  const handleListen = (e?: React.MouseEvent, text?: string) => {
    if (e) e.stopPropagation();
    if (!window.speechSynthesis) return;
    window.speechSynthesis.cancel();
    
    const targetText = text || currentCard?.front || "";
    // Clean up basic markdown symbols for better reading
  ...
  const handleReportError = React.useCallback(async () => {
    ...
    try {
      const cardIndex = deck.cards.findIndex((c: any) => c.id === currentCard.id) + 1;
  ...
  const handleMark = React.useCallback((remembered: boolean) => {
    initAudio();
    if (currentCard) {
      if (remembered) {
         playCorrectSound();
  ...
  }, [currentCard, deck, sessionCorrectCount, sessionHistory.length, isPinned, currentIndex, studyQueue.length, sessionStartTime]);
  ...
  }, [finished, currentCard, isExtracting, isEditing, isFlipped, deckId, currentIndex, sessionCorrectCount, sessionHistory.length]); // Keep all necessary dependencies
        ...
        <div className="glass p-6 md:p-10 rounded-3xl max-w-5xl w-full space-y-8 relative z-10">
          ...
          <div className="text-center space-y-2 relative">
            <motion.div
              ...
              animate={{ scale: [0, 1.2, 1], opacity: 1, rotate: [-20, 10, -5, 0], y: 0 }}
                ...
                <div className="grid grid-cols-4 gap-3 w-full text-center">
                  ...
                  <div className="bg-background/40 p-2 rounded-xl border border-stone-200/50 dark:border-zinc-850">
                    <span className="block text-lg font-black text-amber-600 dark:text-amber-400">
                      {Math.floor(sessionTimeSpent / 60) > 0 ? `${Math.floor(sessionTimeSpent / 60)}m ` : ''}{sessionTimeSpent % 60}s
                    </span>
                    <span className="text-[9px] uppercase font-bold opacity-60 block mt-0.5">Thời gian</span>
                  </div>
                  <div className="bg-background/40 p-2 rounded-xl border border-stone-200/50 dark:border-zinc-850">
            ...
            <div className="lg:col-span-7 bg-stone-200/40 dark:bg-zinc-800/30 p-5 md:p-6 rounded-3xl border border-stone-200 dark:border-zinc-800 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                  {sessionHistory.filter(item => item.status === 'incorrect').length > 0 ? (
                    sessionHistory.filter(item => item.status === 'incorrect').map((item, idx) => (
                      <div 
                        key={idx}
                      ...
                      >
                        ...
                        <p className="font-semibold text-stone-800 dark:text-stone-200 line-clamp-2" title={item.front}>
                          {item.front}
                        </p>
                      </div>
                    ))
                    ...
                    <div className="col-span-full p-4 text-center rounded-xl bg-green-500/10 border border-green-500/20 text-green-700 dark:text-green-400 text-sm font-medium">
                      Tuyệt vời! Bạn không gặp khó khăn với thẻ nào trong phiên này.
                    </div>
                  )}
                </div>
              </div>
    ...
    <div className="max-w-xl mx-auto space-y-6">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <button onClick={handleToggleMute} className="p-2 bg-stone-200/60 dark:bg-zinc-800/50 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition" title={!isSoundEnabled ? "Unmute sounds" : "Mute sounds"}>
            {!isSoundEnabled ? <VolumeX className="w-4 h-4 text-red-500" /> : <Volume2 className="w-4 h-4 opacity-70" />}
          </button>
      ...
      <div className="perspective-1000 relative w-full min-h-[400px] h-[400px] md:h-[450px] cursor-pointer group" onClick={handleFlip}>
        <motion.div 
                ...
                <div className="relative flex flex-col items-center justify-center min-h-[60px] w-full px-8">
                  {currentCard?.wordForm && (
                     <span className={cn(
                        ...
                        currentCard.wordForm.toLowerCase().includes("noun") || currentCard.wordForm.toLowerCase() === "n" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-200 dark:border-blue-800/50" :
                        currentCard.wordForm.toLowerCase().includes("verb") || currentCard.wordForm.toLowerCase() === "v" ? "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border border-red-200 dark:border-red-800/50" :
                        currentCard.wordForm.toLowerCase().includes("adj") ? "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-200 dark:border-amber-800/50" :
                        currentCard.wordForm.toLowerCase().includes("adv") ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50" :
                        currentCard.wordForm.toLowerCase().includes("idiom") || currentCard.wordForm.toLowerCase().includes("colloc") ? "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 border border-purple-200 dark:border-purple-800/50" :
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full relative">
                    <h2 className="text-4xl font-display font-semibold text-center max-w-[85%]"><ReactMarkdown remarkPlugins={[remarkMath]} rehypePlugins={[rehypeKatex]}>{currentCard?.front || (currentCard as any)?.word || "Chưa có nội dung mặt trước. Vui lòng cập nhật."}</ReactMarkdown></h2>
                    <button 
                      onClick={(e) => handleListen(e, currentCard?.front || (currentCard as any)?.word || "")} 
                      ...
                      title="Nghe phát âm (Tiếng Anh) [Phím L]"
                    >
          ...
          <div className="absolute inset-0 backface-hidden [transform:rotateY(180deg)] bg-white dark:bg-zinc-900 border border-stone-200/50 dark:border-zinc-800/50 shadow-xl rounded-3xl text-center h-full overflow-y-auto transform-gpu">
            <div className="w-full min-h-full flex flex-col items-center justify-center p-8 text-lg opacity-90">
              ...
              <div className="w-full flex-1 flex flex-col items-center justify-center py-4">
                 <ParsedTextContent text={currentCard?.back || "Chưa có thông tin mặt sau"} />
              </div>
            ) : (
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 border-amber-600/20 dark:border-amber-500/30">
          <div className="flex justify-center gap-6 md:gap-8 items-center">
            <div className="flex flex-col items-center gap-2">
              <button onClick={() => handleMark(false)} title="Chưa thuộc (Đánh dấu X) [Phím Left]" className="w-16 h-16 rounded-full bg-red-500/10 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition shadow-sm hover:scale-105 active:scale-95">
                <X className="w-8 h-8" />
              </button>
              <span className="text-[10px] uppercase font-bold text-stone-400 dark:text-stone-500 flex items-center gap-1 tracking-widest"><kbd className="px-1 py-0.5 border border-stone-300 dark:border-stone-700 rounded-md font-mono text-[9px]">←</kbd> Quên</span>
            </div>
            
            <div className="flex flex-col items-center gap-2 mt-4">
              <button onClick={handleRemindLater} title="Remind me later (Nhắc lại sau) [Phím Down]" className="w-12 h-12 rounded-full bg-blue-500/10 text-blue-500 flex items-center justify-center hover:bg-blue-500 hover:text-white transition shadow-sm hover:scale-105 active:scale-95">
                <BellPlus className="w-5 h-5" />
              </button>
              <span className="text-[10px] uppercase font-bold text-stone-400 dark:text-stone-500 flex items-center gap-1 tracking-widest"><kbd className="px-1 py-0.5 border border-stone-300 dark:border-stone-700 rounded-md font-mono text-[9px]">↓</kbd> Sau</span>
            </div>

            <div className="flex flex-col items-center gap-2">
              <button onClick={() => handleMark(true)} title="Đã thuộc (Đánh dấu Check) [Phím Right]" className="w-16 h-16 rounded-full bg-green-500/10 text-green-500 flex items-center justify-center hover:bg-green-500 hover:text-white transition shadow-sm hover:scale-105 active:scale-95">
                <Check className="w-8 h-8" />
              </button>
              <span className="text-[10px] uppercase font-bold text-stone-400 dark:text-stone-500 flex items-center gap-1 tracking-widest"><kbd className="px-1 py-0.5 border border-stone-300 dark:border-stone-700 rounded-md font-mono text-[9px]">→</kbd> Nhớ</span>
            </div>
          </div>
          <div className="text-center mt-3 opacity-50 text-[11px] font-medium hidden sm:block">
            Mẹo: Nhấn <kbd className="px-1.5 py-0.5 bg-stone-300 dark:bg-zinc-800 rounded font-mono text-[10px] text-stone-600 dark:text-stone-300 shadow-sm border border-stone-400 dark:border-stone-600">Space</kbd> hoặc <kbd className="px-1.5 py-0.5 bg-stone-300 dark:bg-zinc-800 rounded font-mono text-[10px] text-stone-600 dark:text-stone-300 shadow-sm border border-stone-400 dark:border-stone-600">↑</kbd> để lật thẻ, <kbd className="px-1.5 py-0.5 bg-stone-300 dark:bg-zinc-800 rounded font-mono text-[10px] text-stone-600 dark:text-stone-300 shadow-sm border border-stone-400 dark:border-stone-600">L</kbd> để nghe phát âm
          </div>
          <div className="flex justify-center mt-4">
             <button onClick={handleReportError} className="text-[11px] font-medium text-red-500/80 hover:text-red-500 dark:text-red-400/80 dark:hover:text-red-400 transition flex items-center gap-1.5 bg-red-500/10 hover:bg-red-500/20 px-3 py-1.5 rounded-full">
                         ...
                         <button
                           onClick={(e) => {
                               ...
                               const event = new CustomEvent("trigger-agent3", {
                                   detail: {
                                       message: "Hãy giải thích sâu hơn nữa, hoặc cho tôi ví dụ tình huống về nội dung thẻ này.",
        ...
        <div className="glass p-6 rounded-2xl border border-stone-200 dark:border-zinc-800 mt-12 animate-in fade-in slide-in-from-bottom-8 duration-300">
          ...
          <div className="mb-6 p-4 bg-stone-100/60 dark:bg-zinc-800/40 rounded-xl border border-stone-200/50 dark:border-zinc-700/30">
             <div className="flex justify-between items-center mb-3">
                <h4 className="font-bold text-sm text-stone-700 dark:text-stone-300">Thông tin bộ bài (Metadata)</h4>
                {!isEditingMetadata ? (
                   <button onClick={() => setIsEditingMetadata(true)} className="text-xs flex items-center gap-1 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-bold transition">
                      <Edit3 className="w-3.5 h-3.5" /> Chỉnh sửa
                   </button>
                ) : (
                   <button onClick={() => setIsEditingMetadata(false)} className="text-xs text-stone-500 hover:text-stone-700 dark:hover:text-stone-300 font-bold transition">
                      Hủy bỏ
                   </button>
                )}
             </div>
             ...
             {!isEditingMetadata ? (
                <div className="space-y-1">
                   <p className="text-sm"><span className="font-bold opacity-60">Tên bộ bài:</span> {typeof deck?.title === 'string' ? deck.title : JSON.stringify(deck?.title)}</p>
                   <p className="text-sm"><span className="font-bold opacity-60">Phân loại (Category):</span> <span className="bg-stone-200 dark:bg-zinc-700 px-2 py-0.5 rounded text-xs uppercase tracking-wider">{typeof deck?.subject === 'string' ? deck.subject : (deck?.subject ? JSON.stringify(deck.subject) : "Mặc định")}</span></p>
                </div>
             ) : (
                <div className="space-y-3">
                   <div>
                      <label className="text-xs font-bold opacity-60 uppercase mb-1 block">Tên bộ bài</label>
                      <input 
                         type="text" 
                         value={deckEditTitle}
                   ...
                   <div>
                      <label className="text-xs font-bold opacity-60 uppercase mb-1 block">Phân loại (Category)</label>
                      <select
                         value={deckEditSubject}
                         onChange={(e) => setDeckEditSubject(e.target.value)}
                         className="w-full px-3 py-2 rounded-lg bg-white dark:bg-black border border-stone-300 dark:border-zinc-700 outline-none focus:ring-2 focus:ring-amber-500 text-sm appearance-none"
                      >
                         ...
                         <option value="literature">Văn Học (Literature)</option>
                         <option value="programming">Lập Trình (Programming)</option>
                      </select>
                   </div>
                   <div className="pt-2">
                      <button 
                      ...
                      >
                         {isSavingMetadata ? <div className="w-4 h-4 rounded-full border-2 border-white border-t-transparent animate-spin" /> : <Check className="w-4 h-4" />}
                         Lưu thông tin
                      </button>
                   </div>
                </div>
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-stone-800 dark:text-stone-200">
                    <div>
                      <span className="font-bold text-xs uppercase opacity-40 block">Mặt trước: {c.wordForm && <span className="ml-1 text-[10px] bg-stone-300 dark:bg-zinc-700 px-1 py-0.5 rounded text-stone-700 dark:text-zinc-300">[{c.wordForm}]</span>}</span>
